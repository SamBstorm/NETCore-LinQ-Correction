...
namespace ExerciceDeLinq
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            //             select new { s.First_Name, s.BirthDate, s.Login, s.Year_Result };

            //foreach (var item in result)
            //{
            //    Console.WriteLine($"{item.First_Name} {item.Login} {item.BirthDate} {item.Year_Result}");
            //}
            */

            /** 1.3 

            //IEnumerable<string> result = dc.Students.Select(s => $"{s.Student_ID}|{s.First_Name}|{s.Last_Name}|{s.Section_ID}|{s.Login}|{s.BirthDate}|{s.Year_Result}|{s.Course_ID}");

            //IEnumerable<string> result = from s in dc.Students
            //                             select $"{s.Student_ID}|{s.First_Name}|{s.Last_Name}|{s.Section_ID}|{s.Login}|{s.BirthDate}|{s.Year_Result}|{s.Course_ID}";
            ...
            */
            ...
            //                .Select(s => new { s.Last_Name, s.Year_Result, Status = (s.Year_Result) < 12 ? "KO" : "OK" });

            //var result = from s in dc.Students
            //             where s.BirthDate.Year < 1955
            //             select new { s.Last_Name, s.Year_Result, Status = (s.Year_Result) < 12 ? "KO" : "OK" };
            ...
            */
            ...
            //foreach (var item in result)
            //{
            //    Console.WriteLine($"{item.Full_name} {item.Year_Result}");
            //}
            */
            ...
            //                //.Where(s => s.Section_ID > 1299 && s.Section_ID < 1400)
